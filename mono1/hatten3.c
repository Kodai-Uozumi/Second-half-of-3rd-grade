...
#endif
{
	...
	char *name_img_out = "hatten3.pgm" ;  /* 出力画像ファイル名 */
	GRAY_IMAGE *image_in ; 
	...
	for(y = 0; y < image_in->rows; y++){
	  for(x =0; x < image_in->cols; x++){
	    if((x == 0) || (x == image_in->cols - 1) || (y == 0) || (y == image_in->rows - 1)){
	      image_in->p[y][x] = image_out->p[y][x];
	    }
	    else{
	      a[0] = image_out->p[y - 1][x - 1];
	      a[1] = image_out->p[y - 1][x];
	      a[2] = image_out->p[y - 1][x + 1];
	      a[3] = image_out->p[y][x - 1];
	      a[4] = image_out->p[y][x];
	      a[5] = image_out->p[y][x + 1];
	      a[6] = image_out->p[y + 1][x - 1];
	      a[7] = image_out->p[y + 1][x];
	      a[8] = image_out->p[y + 1][x + 1];
	      for(i = 0; i < 8; i++){
		for(j = 8; j > i; j--){
		  if(a[j - 1] > a[j]){
		    tmp = a[j];
