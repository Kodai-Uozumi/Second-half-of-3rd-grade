...
#endif
{
	...
	char *name_img_out = "enshu3Sobel.pgm" ;  /* 出力画像ファイル名 */
	GRAY_IMAGE *image_in ; 
	...
	for(y = 0; y < image_in->rows; y++){
	  for(x =0; x < image_in->cols; x++){
	    if((x == 0) || (x == image_in->cols - 1) || (y == 0) || (y == image_in->rows - 1)){
	      ...
	      fx = - image_in->p[y - 1][x - 1] + image_in->p[y - 1][x + 1] - 2 * image_in->p[y][x - 1] + 2 * image_in->p[y][x + 1] - image_in->p[y + 1][x - 1] + image_in->p[y + 1][x + 1];
	      fy = - image_in->p[y - 1][x - 1] - 2 * image_in->p[y - 1][x] - image_in->p[y - 1][x + 1] + image_in->p[y + 1][x - 1] + 2 * image_in->p[y + 1][x] + image_in->p[y + 1][x + 1];
